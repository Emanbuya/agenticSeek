...
class Action(Enum):
    def make_newsearch_prompt(self, user_prompt: str, search_result: dict) -> str:
        ...
        quick_queries = ['weather', 'temperature', 'stock', 'price', 'time', 'date']
        is_quick = any(q in user_prompt.lower() for q in quick_queries)
        ...
        if is_quick:
            ...
            IMPORTANT: First check if you can answer this question directly from the search result snippets.

            If the snippets contain the answer (e.g., weather info, stock price, etc.):
            - Take a note with the answer
            - Say {{Action.REQUEST_EXIT.value}} to provide the answer immediately

            If snippets are insufficient:
            - Navigate to the most relevant link

            Do not browse unnecessarily if the answer is in the snippets!
            """
        else:
            return f"""
